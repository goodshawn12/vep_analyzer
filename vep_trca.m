...
% define parameters
CH_list =  [1:32]; %[12,18:20]; %, [5,10:14,18:20], [5,9:15,17:21,24:26], [1:15,17:21,24:26], [1:32]};
data_len_list = 3.4;   % sec
data_offset_list = 0;   % sec
is_ensemble = 1;    % use ensemble classifier (one classifier for each code seq)

% define conditions for testing
var_list = data_len_list;    % different offset values % different channel combinations
...
for var_i = 1:length(var_list)
    ...
    data_range = floor(data_offset_list*SRATE) + (1:1:floor(data_len*SRATE));
...
if plot_fig
    ...
    xlabel('Training data offset (sec)'); ylabel('Cross validation accuracy (''%)');
    set(gca,'fontsize',12)
    ylim([0 100]);
end
